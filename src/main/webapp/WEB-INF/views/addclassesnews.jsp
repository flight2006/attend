			...
			<div class="main-content">
				...
				<div class="page-content">
					...
					<div class="row-fluid">
						...
						<div class="span12">
							<!--PAGE CONTENT BEGINS-->
							<form action="adddepartmentnews" id="adddepartmentnews" method="post">
									<label for="form-field-8">为 <b>${cls.name}</b> 发布部门动态（部门主管:<b>${cls.manager}</b>） </label>
							...
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<c:forEach items="${cnlist}"  var="cn" varStatus="sta" >
									<tr>
										...
										<td>${cn.insertTime}</td>
										<td>${cn.content}</td>
										<td><button class="btn btn-mini btn-danger" onclick="if(window.confirm('确认删除这条动态？')==true)location.href='<%=request.getContextPath() %>/manager/deleteclassesnews?departmentid=${cn.departmentId}&id=${cn.id}'"><i class="icon-remove"></i>删除动态</button></td>
									</tr>
								</c:forEach>
								</tbody>
							</table>
